...
image-name := "harbor.cyverse.org/de/maintenance-page"
tag := "latest"
...
maintenance-page:
    go build -o bin/maintenance-page cmd/maintenance-page/*.go
...
clean:
    #!/usr/bin/env bash
    go clean
    if [ -f bin/maintenance-page ]; then
        rm bin/maintenance-page
    fi
